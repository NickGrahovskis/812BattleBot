...
void canStart() {
  int front_disRange[] = {5, 15};
  // was 15 and 5 
  updateSensorFront();
  if (FRONT_DIS < front_disRange[1] && FRONT_DIS > front_disRange[0]) {
    start = true;
  } else {
    start = false;
  }
...
void rescue() {
  ...
  // Check if either the left or right wheel has moved (pulse count has changed)
  ...
  if (pulseCountLeft != lastLRotation || pulseCountRight != lastRRotation) {
    ...
    // If there is no change in pulses move backwards
    moveBackward(500);
    lastRotationTime = millis();
    return;
...
// Mantain 10 cm from the right wall
int calculateSteeringError() {
  updateSensorRight();
  int rightDisRange[] = {9, 11};

  int targetDistance = 10; // Target wall distance is 10 cm
  if (RIGHT_DIS > rightDisRange[0] && RIGHT_DIS < rightDisRange[1]) {
...
void adjustSteering() {
  ...
  rescue(); // Move backward when stuck
